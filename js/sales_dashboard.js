...
function renderSummaryReport(data) {
    // Clear previous data
    $('#summaryReportHeader').empty();
    $('#summaryReportBody').empty();

    // Check if data exists
    if (!data || !data.sales || data.sales.length === 0) {
        $('#summaryReportBody').html('<tr><td colspan="100" class="text-center py-5 text-muted">No data available for the selected criteria</td></tr>');
        return;
    }

    // Extract unique branches and models
    const branches = [...new Set(data.sales.map(item => item.branch))].sort();
    const models = [...new Set(data.sales.map(item => item.model))].sort();
    
    // Create header row
    let headerRow = '<tr><th>Model/Branch</th>';
    branches.forEach(branch => {
        headerRow += `<th>${branch}</th>`;
    });
    headerRow += '<th>Total</th></tr>';
    $('#summaryReportHeader').html(headerRow);
    ...
    models.forEach(model => {
        ...
        branches.forEach(branch => {
            const sale = data.sales.find(s => s.branch === branch && s.model === model);
            const qty = sale ? parseInt(sale.qty) : 0;
            row += `<td>${qty}</td>`;
            modelTotal += qty;
        });

        row += `<td><strong>${modelTotal}</strong></td></tr>`;
        $('#summaryReportBody').append(row);
    });
    ...
    branches.forEach(branch => {
        const branchTotal = data.sales
            .filter(s => s.branch === branch)
            .reduce((sum, s) => sum + parseInt(s.qty), 0);
        footerRow += `<td><strong>${branchTotal}</strong></td>`;
        grandTotal += branchTotal;
    });

    footerRow += `<td><strong>${grandTotal}</strong></td></tr>`;
    $('#summaryReportBody').append(footerRow);
}
